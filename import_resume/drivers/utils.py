...
def extract_hyperlink_from_cell(cell: dict[str, Any], row_idx: int) -> str | None:
    ...
    if not url and "userEnteredValue" in cell:
        ...
        if "formulaValue" in user_value:
            ...
            if match:
                url = match.group(1)
                logger.debug(
                    f"Found HYPERLINK formula in userEnteredValue in row {row_idx + 2}: {url}"
                )
    ...
    if not url and "effectiveValue" in cell:
        ...
        if "hyperlink" in eff_value:
            ...
            logger.debug(
                f"Found hyperlink in effectiveValue in row {row_idx + 2}: {url}"
            )
        ...
        for chip_run in cell["chipRuns"]:
            if "chip" in chip_run and "richLinkProperties" in chip_run["chip"]:
                ...
                if "uri" in rich_link:
                    url = rich_link["uri"]
                    logger.debug(
                        f"Found Drive file smart chip in row {row_idx + 2}: {url}"
                    )
                    break
    ...
    if not url and "textFormatRuns" in cell and cell["textFormatRuns"]:
        for text_run in cell["textFormatRuns"]:
            if "link" in text_run and "uri" in text_run["link"]:
                ...
                logger.debug(
                    f"Found hyperlink in textFormatRuns in row {row_idx + 2}: {url}"
                )
                break
...
def get_hyperlinks_from_worksheet(
    ...
    try:
        ...
        if column_range:
            ...
            logger.debug(
                f"Fetching hyperlinks from range '{range_str}' in worksheet '{worksheet_title}'"
            )
        else:
            ...
            if column_index:
                logger.debug(
                    f"Fetching hyperlinks from column {column_index} (1-based) in worksheet '{worksheet_title}'"
                )
            else:
        ...
        logger.debug(
            f"API response type: {type(result)}, keys: {result.keys() if isinstance(result, dict) else 'N/A'}"
        )
        ...
        if result and "sheets" in result and len(result["sheets"]) > 0:
            ...
            if "data" in sheet_data and len(sheet_data["data"]) > 0:
                ...
                logger.debug(
                    f"Found {len(row_data)} rows in worksheet '{worksheet_title}'"
                )
        ...
        logger.warning(
            f"Failed to extract hyperlinks from worksheet '{worksheet_title}': {e}"
        )
